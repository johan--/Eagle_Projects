...
PRGRM=$(zenity --width=500 --height=400 --list \
...
ans=$(zenity --width=500 --height=400 --list \
...
case "$ans" in
	...
	./avrdude/avrdude -C ./avrdude/avrdude.conf -c $PROGRAMMER -p m328p -e -u -U lock:w:0x3f:m -U efuse:w:0x05:m -U hfuse:w:0xDA:m -U lfuse:w:0xFF:m
		if [ $? == 0 ]
  			then
			echo "Burning HEX file"
			./avrdude/avrdude -C ./avrdude/avrdude.conf -c $PROGRAMMER -p m328p -U flash:w:./hex_files/adaboot/AdaBoot_328_4800baud.hex -U lock:w:0x0f:m   
			else
			echo "Could not set fuse bits.  Exiting" ; exit 1
		fi 
	;;
